...
export default function ProfileImage({
  ...
  const [imageUrl, setImageUrl] = useState<string | null>(
    userSession?.user?.profile?.image || null
  );
    ...
    startUploading(async () => {
      try {
        ...
        if (imgRef.current && completedCrop) {
          ...
          const { data: updatedUser, errorMessage } = await updateProfileImage(
            userSession.user.profile.id,
            imageUrl
          );
          if (errorMessage || !updatedUser) {
        ...
        {!imageUrl && src ? (
          <div className="flex flex-col items-center gap-2">
            ...
            <div className="flex flex-row gap-2">
              ...
              <Button
                ...
                onClick={() => {
                  setSrc(null);
                  setImageUrl(userSession?.user?.profile?.image || null);
                }}
              >
                <XIcon />
                Cancel
          ...
          <>
            ...
            <Button
              ...
              disabled={!userSession?.user?.profile?.image}
              className="mx-auto mt-2 w-full"
            >
              <XIcon />
              Cancel
